...
class _TestAppState extends State<TestApp> {
  ...
  Future<Warp> fetchData(String query) async {
    ...
    if (json['retcode']==0) {
      return Warp.fromJson(json['data']['list'][0]);
    }
    else {
        ...
        body: Container(
          ...
          child: Column(
              children: [
                CupertinoButton(
                  onPressed: () async {
                    warpData = await fetchData(_url);
                    setState(() {});
                  },
                  child: Text('Submit'),
                ),
                Text('Name: ${warpData.name}'),
                Text('Rarity: ${warpData.rankType}'),
                Text('ID: ${warpData.id}'),
                Text('Time: ${warpData.time}')
            ]),
          )
        )
      ),
